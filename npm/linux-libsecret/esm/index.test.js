...
import { Gio, isGioAvailable, isLinuxKeyringAvailable, Libsecret, LibsecretErrorHandle, } from "./ffi.js";
...
function shouldSkipIntegration(error) {
...
test("linux-libsecret::cancellable handles have an owned native lifecycle", {
    ...
    const schema = Libsecret.secretSchemaNew("org.freedesktop.Secret.Generic", 0, "service", 0, "account", 0, null);
    if (schema === null)
    ...
    try {
        ...
        const errorOut = new LibsecretErrorHandle();
        const password = Libsecret.secretPasswordLookupSync(schema, cancellable, errorOut, "service", "test", "account", "test", null);
        equal(password, null);
...
test("linux-libsecret::set/get/list/delete roundtrip (dangerous)", { skip: !LINUX || !dangerousMutations || !isAvailable() || "Bun" in globalThis }, (t) => {
