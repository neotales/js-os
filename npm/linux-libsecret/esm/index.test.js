...
import { getSecretBytes, isLinuxLibsecretAvailable, listSecrets, readSecret, removeSecret, saveSecret, } from "./index.js";
...
test("linux-libsecret::set/get/list/delete roundtrip (dangerous)", { skip: !LINUX || !DANGEROUS_MUTATIONS || "Bun" in globalThis || !isLinuxLibsecretAvailable() }, (t) => {
    ...
    try {
        ...
        if (shouldSkipIntegration(error)) {
            t.skip(`Integration environment unavailable: ${error instanceof Error ? error.message : String(error)}`);
            return;
        }
        throw error;
    }
