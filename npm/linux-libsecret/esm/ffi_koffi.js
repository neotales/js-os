...
const secret_password_lookup_sync = libsecret.func("str secret_password_lookup_sync(void *schema, void *cancellable, _Out_ void **error, const char *attr1, const char *val1, const char *attr2, const char *val2, void *end)");
...
function getSchema() {
    if (schemaPtr !== null)
        return schemaPtr;
    schemaPtr = secret_schema_new("org.freedesktop.Secret.Generic", 0, "service", 0, "account", 0, null);
    return schemaPtr;
}
...
export const backend = {
    getSecretBytes(service, account) {
        const errorOut = [null];
        const value = secret_password_lookup_sync(getSchema(), null, errorOut, "service", service, "account", account, null);
    ...
    setSecretBytes(service, account, secret) {
        const errorOut = [null];
        const ok = secret_password_store_sync(getSchema(), "default", `${service}/${account}`, dec.decode(secret), null, errorOut, "service", service, "account", account, null);
        throwIfError(errorOut);
        if (!ok)
    ...
    deleteSecret(service, account) {
        const errorOut = [null];
        const ok = secret_password_clear_sync(getSchema(), null, errorOut, "service", service, "account", account, null);
        throwIfError(errorOut);
        return !!ok;
    },
    list(serviceName) {
        const errorOut = [null];
        const list = secret_password_search_sync(getSchema(), 2, null, errorOut, "service", serviceName, null);
        throwIfError(errorOut);
        if (!list)
            return [];
        const results = [];
