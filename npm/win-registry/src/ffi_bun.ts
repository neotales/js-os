...
function ensureLoaded(): BunFfiLibrary {
  ...
  library = dlopen("advapi32.dll", SYMBOLS) as unknown as BunFfiLibrary;
...
export const backend: RegistryBackend = {
   ...
   */
  closeKey(hkey) {
    fns().RegCloseKey(hkeyToPtr(hkey) as Pointer);
  },
  ...
  deleteKey(hkey, subKey) {
    return fns().RegDeleteKeyW(
      hkeyToPtr(hkey) as Pointer,
      ptr(stringToWide(subKey)),
    );
  },
  /**
   * Deletes a value.
