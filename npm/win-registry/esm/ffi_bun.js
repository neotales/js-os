...
export const backend = {
    ...
    createKey(hkey, subKey, access) {
        ...
        const status = symbols.RegCreateKeyExW(hkeyToPtr(hkey), ptr(wSubKey), 0, null, 0, access, null, ptr(handleBuf), ptr(dispositionBuf));
        if (status !== ERROR_SUCCESS)
    ...
    queryInfoKey(hkey) {
        ...
        const status = symbols.RegQueryInfoKeyW(hkeyToPtr(hkey), null, null, null, ptr(subKeyCountBuf), ptr(maxSubKeyLenBuf), null, ptr(valueCountBuf), ptr(maxValueNameLenBuf), ptr(maxValueLenBuf), null, ptr(lastWriteTimeBuf));
        if (status !== ERROR_SUCCESS)
    ...
    enumKeyNames(hkey, index, nameBufferSize) {
        ...
        const status = symbols.RegEnumKeyExW(hkeyToPtr(hkey), index, ptr(nameBuf), ptr(sizeBuf), null, null, null, null);
    ...
    enumValueNames(hkey, index, nameBufferSize) {
        ...
        const status = symbols.RegEnumValueW(hkeyToPtr(hkey), index, ptr(nameBuf), ptr(sizeBuf), null, ptr(typeBuf), null, null);
        if (status === ERROR_NO_MORE_ITEMS)
            return null;
        if (status !== ERROR_SUCCESS)
    ...
    queryValue(hkey, valueName) {
        ...
        let status = symbols.RegQueryValueExW(hkeyToPtr(hkey), ptr(wName), null, ptr(typeBuf), ptr(buffer), ptr(sizeBuf));
        if (status === ERROR_MORE_DATA) {
            const needed = readU32(sizeBuf);
            const bigBuf = new Uint8Array(needed);
            writeU32(sizeBuf, needed);
            status = symbols.RegQueryValueExW(hkeyToPtr(hkey), ptr(wName), null, ptr(typeBuf), ptr(bigBuf), ptr(sizeBuf));
            if (status !== ERROR_SUCCESS)
                return null;
    ...
    setValue(hkey, valueName, type, data) {
        const status = symbols.RegSetValueExW(hkeyToPtr(hkey), ptr(stringToWide(valueName)), 0, type, ptr(data), data.length);
        if (status !== ERROR_SUCCESS)
