...
import { HKEY_CURRENT_USER, HKEY_LOCAL_MACHINE, multiStringToWide, parseRegistryPath, stringToWide, wideToMultiString, wideToString, } from "./types.js";
...
test("win-registry::Registry can read well-known Windows registry values", { skip: !WINDOWS }, () => {
    if (!WINDOWS)
        return;
...
test("win-registry::Registry create/open/set/get/delete (Windows, dangerous)", { skip: !WINDOWS || !DANGEROUS_MUTATIONS }, () => {
...
test("win-registry::createKey relative operations (Windows, dangerous)", { skip: !WINDOWS || !DANGEROUS_MUTATIONS }, () => {
    if (!WINDOWS || !DANGEROUS_MUTATIONS)
        return;
