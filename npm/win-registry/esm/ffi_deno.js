...
function ensureLoaded() {
    ...
    try {
        library = getDeno().dlopen("advapi32.dll", SYMBOLS);
    }
    catch (error) {
        const reason = error instanceof Error ? error.message : String(error);
        throw new RegistryError(`Failed to open "advapi32.dll" via Deno FFI (requires --allow-ffi): ${reason}`, { cause: error });
    }
    return library;
}
/**
...
export const backend = {
    ...
    openKey(hkey, subKey, access) {
        ...
        const status = fns().RegOpenKeyExW(ptr(hkey), wSubKey, 0, access, resultBuf);
    ...
    createKey(hkey, subKey, access) {
        ...
        const status = fns().RegCreateKeyExW(ptr(hkey), wSubKey, 0, null, 0, access, null, handleBuf, dispositionBuf);
        if (status !== ERROR_SUCCESS) {
    ...
    queryInfoKey(hkey) {
        ...
        const status = fns().RegQueryInfoKeyW(ptr(hkey), null, null, null, subKeyCountBuf, maxSubKeyLenBuf, null, valueCountBuf, maxValueNameLenBuf, maxValueLenBuf, null, lastWriteTimeBuf);
        if (status !== ERROR_SUCCESS) {
    ...
    enumKeyNames(hkey, index, nameBufferSize) {
        ...
        const status = fns().RegEnumKeyExW(ptr(hkey), index, nameBuf, sizeBuf, null, null, null, null);
    ...
    enumValueNames(hkey, index, nameBufferSize) {
        ...
        const status = fns().RegEnumValueW(ptr(hkey), index, nameBuf, sizeBuf, null, typeBuf, null, null);
        if (status === ERROR_NO_MORE_ITEMS) {
            return null;
        }
        if (status !== ERROR_SUCCESS) {
    ...
    queryValue(hkey, valueName) {
        ...
        let status = fns().RegQueryValueExW(ptr(hkey), wName, null, typeBuf, buffer, sizeBuf);
        if (status === ERROR_MORE_DATA) {
            const needed = readU32(sizeBuf);
            const bigBuf = new Uint8Array(needed);
            writeU32(sizeBuf, needed);
            status = fns().RegQueryValueExW(ptr(hkey), wName, null, typeBuf, bigBuf, sizeBuf);
            if (status !== ERROR_SUCCESS) {
                return null;
            }
            return {
    ...
    setValue(hkey, valueName, type, data) {
        const status = fns().RegSetValueExW(ptr(hkey), stringToWide(valueName), 0, type, data, data.length);
        if (status !== ERROR_SUCCESS) {
