...
export const backend = {
    openKey(hkey, subKey, access) {
        ...
        const status = symbols.RegOpenKeyExW(Deno.UnsafePointer.create(hkey), wSubKey, 0, access, resultBuf);
    ...
    createKey(hkey, subKey, access) {
        ...
        const status = symbols.RegCreateKeyExW(Deno.UnsafePointer.create(hkey), wSubKey, 0, null, 0, access, null, handleBuf, dispositionBuf);
        if (status !== ERROR_SUCCESS)
    ...
    queryInfoKey(hkey) {
        ...
        const status = symbols.RegQueryInfoKeyW(Deno.UnsafePointer.create(hkey), null, null, null, subKeyCountBuf, maxSubKeyLenBuf, null, valueCountBuf, maxValueNameLenBuf, maxValueLenBuf, null, lastWriteTimeBuf);
        if (status !== ERROR_SUCCESS)
    ...
    enumKeyNames(hkey, index, nameBufferSize) {
        ...
        const status = symbols.RegEnumKeyExW(Deno.UnsafePointer.create(hkey), index, nameBuf, sizeBuf, null, null, null, null);
    ...
    enumValueNames(hkey, index, nameBufferSize) {
        ...
        const status = symbols.RegEnumValueW(Deno.UnsafePointer.create(hkey), index, nameBuf, sizeBuf, null, typeBuf, null, null);
        if (status === ERROR_NO_MORE_ITEMS)
            return null;
        if (status !== ERROR_SUCCESS)
    ...
    queryValue(hkey, valueName) {
        ...
        let status = symbols.RegQueryValueExW(Deno.UnsafePointer.create(hkey), wName, null, typeBuf, buffer, sizeBuf);
        if (status === ERROR_MORE_DATA) {
            const needed = readU32(sizeBuf);
            const bigBuf = new Uint8Array(needed);
            writeU32(sizeBuf, needed);
            status = symbols.RegQueryValueExW(Deno.UnsafePointer.create(hkey), wName, null, typeBuf, bigBuf, sizeBuf);
            if (status !== ERROR_SUCCESS)
                return null;
    ...
    setValue(hkey, valueName, type, data) {
        const status = symbols.RegSetValueExW(Deno.UnsafePointer.create(hkey), stringToWide(valueName), 0, type, data, data.length);
        if (status !== ERROR_SUCCESS)
