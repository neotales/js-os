...
export const backend = {
    ...
    createKey(hkey, subKey, access) {
        ...
        const status = lib.functions.RegCreateKeyExW(hkey, wSubKey, 0, null, 0, access, null, handleBuf, dispositionBuf);
        if (status !== ERROR_SUCCESS)
    ...
    queryInfoKey(hkey) {
        ...
        const status = lib.functions.RegQueryInfoKeyW(hkey, null, null, null, subKeyCountBuf, maxSubKeyLenBuf, null, valueCountBuf, maxValueNameLenBuf, maxValueLenBuf, null, lastWriteTimeBuf);
        if (status !== ERROR_SUCCESS)
    ...
    enumKeyNames(hkey, index, nameBufferSize) {
        ...
        const status = lib.functions.RegEnumKeyExW(hkey, index, nameBuf, sizeBuf, null, null, null, null);
    ...
    enumValueNames(hkey, index, nameBufferSize) {
        ...
        const status = lib.functions.RegEnumValueW(hkey, index, nameBuf, sizeBuf, null, typeBuf, null, null);
        if (status === ERROR_NO_MORE_ITEMS)
            return null;
        if (status !== ERROR_SUCCESS)
    ...
    setValue(hkey, valueName, type, data) {
        const status = lib.functions.RegSetValueExW(hkey, stringToWide(valueName), 0, type, data, data.length);
        if (status !== ERROR_SUCCESS)
