...
function buildCredentialBuffer(cred) {
    ...
    refs.push(blob);
    if (blob.length > 0)
        view.setBigUint64(OFF_BLOB, ffi.getRawPointer(blob), true);
    ...
    refs.push(wAlias);
    if (cred.targetAlias)
        view.setBigUint64(OFF_TARGET_ALIAS, ffi.getRawPointer(wAlias), true);
    const wUser = stringToWide(cred.userName);
    refs.push(wUser);
    if (cred.userName)
        view.setBigUint64(OFF_USER_NAME, ffi.getRawPointer(wUser), true);
...
export const backend = {
    ...
    enumerate(filter, flags) {
        ...
        const ok = lib.functions.CredEnumerateW(filter !== null ? stringToWide(filter) : null, flags, countBuf, credsBuf);
        if (!ok)
            return [];
