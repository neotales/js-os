...
const SecKeychainFindGenericPassword = sec.func("int SecKeychainFindGenericPassword(void *keychainOrArray, uint32 serviceNameLength, const void *serviceName, uint32 accountNameLength, const void *accountName, _Out_ uint32 *passwordLength, _Out_ void **passwordData, _Out_ void **itemRef)");
const SecKeychainAddGenericPassword = sec.func("int SecKeychainAddGenericPassword(void *keychain, uint32 serviceNameLength, const void *serviceName, uint32 accountNameLength, const void *accountName, uint32 passwordLength, const void *passwordData, _Out_ void **itemRef)");
const SecKeychainItemModifyAttributesAndData = sec.func("int SecKeychainItemModifyAttributesAndData(void *itemRef, void *attrList, uint32 length, const void *data)");
...
export const backend = {
    ...
    setSecretBytes(service, account, secret) {
        ...
        try {
            if (found?.itemPtr) {
                osCheck(SecKeychainItemModifyAttributesAndData(found.itemPtr, null, secret.length, secret), "SecKeychainItemModifyAttributesAndData failed");
                return;
            }
            const serviceBytes = cbytes(service);
            const accountBytes = cbytes(account);
            const itemOut = [null];
            osCheck(SecKeychainAddGenericPassword(null, serviceBytes.length, serviceBytes, accountBytes.length, accountBytes, secret.length, secret, itemOut), "SecKeychainAddGenericPassword failed");
            if (itemOut[0])
                CFRelease(itemOut[0]);
        }
        finally {
