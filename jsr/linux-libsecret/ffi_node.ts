...
function readCString(pointer: bigint): Uint8Array {
  const bytes: number[] = [];
...
function schemaPointer(handle: SecretSchemaHandle): bigint {
  ...
  return handle.valueOf() as bigint;
}

function passwordPointer(handle: SecretPasswordHandle): bigint {
  if (handle.runtime !== runtime || typeof handle.valueOf() !== "bigint")
    throw new TypeError("Secret password handle belongs to a different runtime.");
  return handle.valueOf() as bigint;
}

function cancellablePointer(handle: GCancellableHandle): bigint {
...
function secretSchemaNew(
  ...
  const pointer = libsecret.functions.secret_schema_new(
    cstr(name),
    flags,
    cstr(attributeName1),
    attributeType1,
...
function secretPasswordLookupSync(
  ...
  const pointer = libsecret.functions.secret_password_lookup_sync(
...
function secretPasswordStoreSync(
  ...
  prepareLibsecretError(errorOut, runtime);
  const errorStorage = new Uint8Array(8);
  const stored = libsecret.functions.secret_password_store_sync(
    schemaPointer(schema),
    cstr(collection),
    cstr(label),
    cstr(password),
...
function secretPasswordClearSync(
  ...
  prepareLibsecretError(errorOut, runtime);
  const errorStorage = new Uint8Array(8);
  const cleared = libsecret.functions.secret_password_clear_sync(
    schemaPointer(schema),
    cancellable === null ? null : cancellablePointer(cancellable),
    errorStorage,
    cstr(attributeName1),
...
function listDataPointer(list: bigint): bigint {
  return ffi.getUint64(list, 0);
}

function listNextPointer(list: bigint): bigint {
