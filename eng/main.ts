...
async function writeWinRegistryDenoPackage(
  ...
  const readme = rebrand(await Deno.readTextFile(join(source, "README.md"))).replace(
    /## Runtime Notes[\s\S]*?(?=\n## License)/,
    '## Runtime Support\n\nThis JSR package supports Deno, Bun, and Node.js on Windows. Run Deno with `--allow-ffi`. Run Node.js with `--experimental-ffi` to use the native `node:ffi` backend. The JSR package has no koffi fallback; use `npm:@neotales/win-registry` when a Node.js project needs the koffi-backed npm package. When the backend cannot load, `isRegistryAvailable()` returns `false` and registry operations throw `RegistryError`.\n\n### Remediation\n\nWhen registry operations throw `RegistryError` with "not supported":\n\n- **Deno**: run with `--allow-ffi`.\n- **Node.js**: use Node >= 26 with `--experimental-ffi`, or install the npm package [`@neotales/win-registry`](https://www.npmjs.com/package/@neotales/win-registry), which bundles a koffi fallback that works without the experimental flag.\n\n## Resource Management\n\nEvery key returned by `Registry.openKey()` or `Registry.createKey()` owns a Windows registry handle. Prefer `using` so the handle closes at the end of its lexical scope, even when an operation throws. Predefined root keys such as `Registry.HKCU` do not need closing.\n\n```ts\nusing key = Registry.openKey("HKCU\\\\Software");\nconsole.log(key.getValueNames());\n```\n\nWhen `using` is unavailable, close the key in `finally`:\n\n```ts\nconst key = Registry.openKey("HKCU\\\\Software");\ntry {\n  console.log(key.getValueNames());\n} finally {\n  key.close();\n}\n```\n',
  );
  const installation =
...
async function importModule(name: string, replace: boolean): Promise<void> {
  if (
    ...
    throw new Error(
      `The Deno-only migration is defined for win-registry, win-cred, darwin-keychain, and linux-libsecret only; review it before importing ${name}.`,
    );
  }
